...
<?php
$dossier = new RecursiveDirectoryIterator("../uploads", RecursiveDirectoryIterator::SKIP_DOTS);
foreach (new RecursiveIteratorIterator($dossier) as $fichier) {
    if ($fichier->isFile()) { 
        $cheminComplet = $fichier->getPathname();
        echo 'Titre du devoir : ' . basename($cheminComplet) . '<br>';
        ...
        echo '<form method="post" action="delete.php" onsubmit="return confirm(\'Êtes-vous sûr de vouloir supprimer ce fichier ?\');">';
        echo '<input type="hidden" name="fichier" value="' . htmlspecialchars($cheminComplet) . '">';
